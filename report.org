...
#+begin_src gnuplot :exports code :results silent
  set grid
  plot 100 * sin(x), ((((x * 2) / (x - 6)) + ((3 * (x + 7)) / (x * (3 - x)))) * 7)
#+end_src
...
download from [[http://cs.unm.edu/~eschulte/classes/cs591-rpc/videos][here]] and saved to your desktop.  On a mac you may need
...
[[http://www.gnu.org/software/emacs/][Emacs]] [[http://orgmode.org/][Org-mode]] and [[http://orgmode.org/worg/org-contrib/babel/org-babel.php][Org-babel]].  These tools combined allow all the code
and processed data used to conduct the experimentation and data
analysis supporting this report to be embedded in the same document
containing the prose of the report.  The source of this document as
well as all supporting files required to re-create these results can
be obtained using either git or a =.tar= or =.zip= download from
http://github.com/eschulte/gp4ixm-report.
