...
analysis supporting this report to be embedded in the same document as
the prose of the report.  The source of this document as well as all
supporting files required to re-create these results can be obtained
as a =.tar= or =.zip= download or using [[http://git-scm.com/][git]] from
